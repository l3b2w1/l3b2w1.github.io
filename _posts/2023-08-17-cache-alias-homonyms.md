...
这会导致B进程的VB对应的高速缓存行中的数据变得过时。这就会导致问题。

只有虚拟地址索引的设计方案（VIVT和VIPT）才可能会导致别名问题。  
说可能是因为虚拟地址索引并不是必然会产生别名问题，需要一定的条件才会产生。  

虚拟页号和页内偏移的划分边界取决于页面大小。  
如果页面大小为4KB，则页内偏移为12位；如果页面大小为16KB，则页内偏移为14位。  

标记与索引的划分取决于cache的总大小（对于直接映射而言是总大小，对于组相连映射则取决于一路cache的总大小）。  
一路cache的大小为16KB时需要14位，为64KB时需要16位（这是索引与行内偏移的位数之和：行内偏移占据低位，cache行大小为64B时占6位，其余高位才是索引位数）。  

当索引位数与行内偏移位数之和超过地址转换划分边界所产生的页内偏移位数时  
（等价于一路cache的大小超过页面大小），就会存在别名问题。  

* 如果索引位数与行内偏移位数之和没有超过页内偏移数，那么虚拟地址索引和物理地址索引必定具有相同的含义，
在地址转换过程中，虚拟地址和物理地址的页内偏移值是相等的，  
因为给定的索引数值在一路cache中的位置是固定的，只有一个cache行对应该索引值，所以不存在别名问题。

* 而如果索引位数与行内偏移位数之和超过了页内偏移数，那么一路cache大小将超过页面大小，  
...
假定地址宽度为64位，那么tag占据50位，index占据8位，cache line size占据6位  
一路cache可以缓存2个PAGE的数据  

采用虚拟索引的匹配方式，两个不同的虚拟地址有可能映射到同一个物理地址  
索引与行内偏移共14bit，超过了页内偏移的13bit，超出的1bit取自虚拟页号，  
因此同一物理页的多个虚拟映射可能落入不同的cache组（即需要2种颜色）
