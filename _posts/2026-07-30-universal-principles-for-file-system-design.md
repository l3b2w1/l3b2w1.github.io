...
> **时代定位**：§0–§13 提炼的是经典磁盘时代（约 1984–1999）原地更新（in-place update）文件系统的设计原则；  
2000 年后的三大演进（SSD、CoW、校验和自愈）见 §14——"普适"的是问题与权衡，不是所有具体解法。
...
5. [目录与索引：B+tree 是盘上索引的主流方案](#5-目录与索引btree-是盘上索引的主流方案)
...
14. [时代边界：2000 年后的演进（SSD / CoW / 校验和）](#14-时代边界2000-年后的演进)
...
- **ext2 选了速度**：不保证调用完成时数据已落盘、也不保证落盘顺序（元数据大多先在内存完成）——换来约一个数量级的性能。  
    注意它并非"不要一致性"：布局确定、有 fsck，只是把一致性重建推迟到崩溃后的**离线 fsck**，而非运行期保障。  
...
> **时代限定**：本节基于 HDD 机械磁盘模型（原书时代）。SSD/NVMe 下"消除寻道"不再是第一目标，但"局部性/批量"多数仍然成立——详见 §14.1 的修正。
...
四个独立实现收敛到同一模式，说明它解决的是通用问题。**但注意复杂度差异**：  
BFS 的 allocation group 是**纯逻辑划分**（无盘上结构、无独立锁、只服务地址编码与布局策略）；  
XFS 的 AG 是**完整的资源管理域**（AGF/AGI/空闲树/i-node 树/后来的 rmapbt/refcountbt + per-AG 锁）。  
两者共享 locality + scalability 思想，实现深度不可同日而语——类比例表时应说清这一点，否则容易高估 BFS 的工程复杂度。
...
**结论（多实现收敛）**：目录与索引的盘上结构**主流选择是 B+tree**——XFS 目录 dabtree、NTFS 目录 B+tree、HFS catalog B*tree、BFS 目录/索引 B+tree。
但"主流"≠"唯一"：**ext4 的 htree**（两级：hash 分桶定位目录块 + 块内按 hash 值排序二分查找，冲突块内线性消解）证明 hash 只有被"驯化"成树形才可用；  
**btrfs 的 B-tree 目录项（hash 作键）**、**ZFS 的 ZAP**、**bcachefs 的 btree** 又各不相同。普适的是约束（盘上保序/可增长/查找高效/耐重复），不是某一种结构。
...
### 原则 6.5：日志空间是天然的竞争点；现代实现以分段/聚合大幅缓解

单一共享日志意味着所有事务竞争同一日志空间与写锁——**在原书时代（BFS 的 log semaphore 模型）这把更新压成了单线程**。  
但"单日志必然串行"是过度推断，扩容与缓解路径有三：
- 多个并行日志流 + 时间戳/LSN 排序回放（多日志方案）；
- 每事务预留固定日志空间、独立管理（XFS 早期方案变体）；
- **把日志"内存化"再批量成形**：现代 XFS 的 CIL（committed item list）先在内存按 LSN 聚合提交项、合并对同一元数据的重复修改，  
   再批量写入 iclog——竞争点从"日志写锁"前移为"内存列表追加"，串行化大幅削弱。ext4 jbd2 的批量 commit 同理。

另一个杠杆：**日志区大小直接决定可缓冲事务量**（BFS 从 512K→2048K 获得可观提升）。
...
| 2.1 | 块组骨架 | AG=位图块倍数 | AGI/AGF+双空闲树+inobt per AG（v5 起 +rmapbt/refcountbt） | GDT+位图+inode 表 per 组 | —（MFT 中心化） |
...
§0–§13 提炼自 1999 年的 HDD 世界。2000 年后三条新路线改变了部分原则的权重，**阅读前文时须带着本节滤镜**。

### 14.1 SSD/NVMe：原则 1 的修正

- **前提变化**：NAND flash 无机械寻道（seek ≈ 0），随机读不再比顺序读慢一个数量级（NVMe 随机读 IOPS 可达数十万）。
- **仍然成立的**：大块/批量/合并 I/O（受限于 queue depth 与总线而非机械臂）；局部性（NAND 页/擦除块粒度使连续写仍优于离散写）。
- **权重下降的**：排序合并写、read-ahead 的收益显著缩小；随机 4K 读不再灾难。
- **新增的约束**：**写放大（write amplification）**成为核心指标——FTL 的擦除-重写使小随机写在物理层被放大；**对齐擦除块边界**、**避免覆盖写**（助 FTL 垃圾回收）成为新的布局考量；尾延迟（GC 引起）比平均延迟更影响体验。
- **修正后的表述**：*"减少随机小 I/O 与写放大"* 取代 *"消除寻道"* 成为布局第一目标。F2FS 的 log-structured 设计、各文件系统对 discard/TRIM 的支持即为此而生。

### 14.2 CoW 文件系统：崩溃一致性的另一条路线

- **思想**（ZFS 2005 / btrfs 2007 / APFS 2016）：**永不原地覆写**——所有修改写到新位置，自底向上级联更新父指针，最后原子切换根（uberblock/tree root）。崩溃任意时刻发生，旧的完整树仍在盘上。
- **与日志的关系**：CoW 把"先写日志再改原地"推广为"一切修改都写新地"——日志是该思想的局部版，CoW 是全集版；ZFS 仍有 ZIL（intent log）处理 fsync 语义。
- **免费副产品**：快照（snapshot）只是保留旧根指针，成本 O(1)——原书时代 LFS"隐式版本化"的潜力在 CoW 中真正实现。
- **代价**：写放大（级联更新路径上所有块）、碎片化随时间加重、空闲空间管理复杂（ZFS metaslab / btrfs extent tree）、删除快照后的空间回收昂贵。
- **对本文原则的影响**：§2 块组（ZFS metaslab、btrfs block group——思想延续）；§6 日志（被根切换事务取代）；§8 预分配（CoW 下预分配反而有害——预留块无法用）；§3 extent（btrfs extent 依然核心）。

### 14.3 校验和与自愈：从"检测停机"到"检测修复"

- **1999 年的做法**（§4.4、§12.1）：magic number + sanity check + 发现损坏即 panic——**只能检测，不能修复**。
- **现代做法**：
  - **元数据校验和**：XFS v5 所有元数据块 CRC32C + 自描述（每块自带 type/owner/块号——"magic number 原则"的完整实现）；ext4 metadata_csum、journal checksum。
  - **端到端数据校验和**（ZFS/btrfs）：每个数据块带校验和，读到坏块时从冗余副本（mirror/RAID）**自动修复**（self-healing）——校验和不仅检测，还指导修复。
  - **在线 scrub**：定期遍历校验，在副本失效前主动发现腐烂（bit rot）。
- **演进逻辑**：检测（magic）→ 定位（自描述元数据，XFS v5 块头含所属对象）→ 修复（校验和 + 冗余副本）。**完整性从"靠测试保证"变成"靠结构保证"。**

### 14.4 本节结论

经典原则的**问题定义**全部仍然有效（一致性 vs 性能、局部性、并发、可测试性）；  
**具体解法**已有替代（CoW 替代原地+日志、校验和替代 magic+panic、SSD 改写 I/O 成本模型）。读前十三章时应问的是："这个原则背后的约束在我的时代还成立吗？"
