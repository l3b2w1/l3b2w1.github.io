...
![多设备与 device table：一个 EROFS 横跨多个块设备](https://raw.githubusercontent.com/l3b2w1/l3b2w1.github.io/master/img/2026-09-18-erofs-31-multidev-usage.svg)

**一句话**：一个 EROFS 可以横跨多个块设备，
镜像里的**地址高位编着设备号**，一次映射就能同时知道"在哪个设备 + 设备内偏移"。

自上而下四层：

| 层 | 回答什么 | 一句话 |
|---|---|---|
| **① 为什么**（淡绿） | 什么时候需要多设备 | 容量不够 / 想按介质拆分 / 想让多个镜像复用同一份基础数据；挂载时用 `device=` 逐个指定 |
| **② 怎么编址**（淡蓝） | 地址和设备表长什么样 | `[设备号 16 位][设备内偏移 48 位]`；主设备 `sbi->dif0` 内嵌，其余用 `idr` 存、`rwsem` 保护 |
| **③ 怎么解析**（淡黄） | `erofs_map_dev()` 的三条分支 | ①默认填主设备 → ②`m_deviceid != 0` 查 `idr_find(id-1)` → ③`m_deviceid == 0` 但有额外设备且非 flat 时遍历区间（最慢） |
| **④ 两种模式**（淡紫） | flat 与非 flat 差在哪 | flat：拼成一个连续空间，`m_pa += uniaddr` 一次加法；非 flat：各自独立，换设备 + `m_pa -= startoff` |

**⚠️ 千万别忘了最后那个红框**：内核里**没有 "layer（层）" 这个概念**。  
容器镜像的"分层"是构建/分发层的语义，EROFS 内核只认「一个文件系统 + 若干设备 + 地址指向哪个设备」。  
把 overlayfs 的分层语义套过来会**完全理解错**。
