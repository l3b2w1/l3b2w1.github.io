...
CMA区域预留内存是给驱动通过DMA方式申请大块连续内存时用的，必须通过`dma_alloc_xxx`这样的固定接口。  
...
比如 `cma=128M@1G`，从1G物理地址开始，预留128M。如果传递`cmd=0`，表示禁止系统预留CMA内存。

**一般不建议自己指定cma范围，留给系统启动阶段自行选择**，因为一旦cma区域和其它专用区域重叠，很容易出现奇怪的问题。
