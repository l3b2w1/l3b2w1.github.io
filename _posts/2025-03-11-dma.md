...
2. DMA首选从CMA分配，没有的话__get_free_pages动态获取。  
   需要打开配置项CONFIG_DMA_CMA，DMA才可以利用CMA分配内存。  
4. 一致性是通过配置内存页表属性保证的，这个属性值MT_NORMAL_NC，NC表示non-cached，普通内存，不经缓存。
5. swiotlb的作用在于，使得寻址能力较低，无法直接寻址到内核所分配的DMA buffer的那些设备，也能够进程DMA操作。
