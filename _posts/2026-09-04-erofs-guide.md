...
EROFS 本身的代码量其实很小——21 个文件、9317 行，在文件系统里算相当精简的。
...
记住这个划分：**EROFS 只负责回答"数据在磁盘哪里"这一件事**（外加压缩文件特有的解压）。  

读磁盘的脏活由 block layer 做，缓存管理由 page cache 做，接口统一由 VFS 做。

这也解释了为什么 EROFS 只有 9317 行——真正属于它的工作不多，大量基础设施是内核提供的。
...
- EROFS 里 folio 的 order 通常是 0（即单页），阶段 2 会看到为什么
...
内核里至少有三个"大小"概念，单位不同，混用会让人抓狂：

| 概念 | 典型大小 | 谁在用 | 说明 |
|---|---|---|---|
| **扇区 sector** | 512 字节 | 硬件 / block layer | 磁盘硬件的最小传输单位 |
| **块 block** | 4096 字节 | 文件系统 | 文件系统的最小分配单位 |
| **页 page** | 4096 字节 | 内存管理 / page cache | 内存管理的最小单位 |

EROFS 的块大小**不是编译期常量**，而是存在 superblock 的 `blkszbits` 字段里——
`super.c:271` 把它读入，紧接着 `super.c:272` 校验合法范围：

```c
sbi->blkszbits = dsb->blkszbits;                                  /* super.c:271 */
if (sbi->blkszbits < 9 || sbi->blkszbits > PAGE_SHIFT) {           /* super.c:272 */
...
iomap 是内核提供的一套**通用框架**，用来表达"文件的这段逻辑范围，对应磁盘的哪个物理范围"。
...
iomap 的做法是：文件系统只实现"**翻译**"这一件事——
"文件偏移 20K~24K 对应磁盘块 1000~1001"。  
至于怎么提交 IO、怎么处理 DAX、怎么支持 FIEMAP，都由 iomap 框架统一处理。
...
EROFS 是 **Read-Only File System**，挂载后不能写。
...
- **约束**：数据布局在 mkfs 时定死，运行时无法调整。
  ...
  这直接导致了 EROFS 有 5 种 datalayout（阶段 1）。
...
下一章：[01-on-disk格式](01-on-disk格式.md)
