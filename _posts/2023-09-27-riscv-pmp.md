...
## PMP介绍
为了支持安全处理并包含故障，有必要限制运行在hart上的软件可以访问的物理地址。  
可选的物理内存保护（PMP）单元提供了每个hart机器模式控制寄存器，允许为每个物理内存区域指定物理内存访问权限（读、写、执行）。  
PMP与PMA联合并行检查。  
...
## U74 core PMP介绍
U7 PMP 支持 8 个区域，最小区域大小为 4096 字节。  


#### PMP 功能描述  
U7 PMP 单元具有 8 个区域，最小粒度为 4096 字节。每个区域的访问由一个 8 位的 pmpXcfg 字段和一个对应的 pmpaddrX 寄存器控制。  
...
但是，锁定的区域（请参阅第 4.8.2 节）还会在 M 模式下强制执行其权限。  
...
此外，SiFive 设计可能包含一个前端端口，允许外部总线主机访问系统的完整内存映射。PMP 无法阻止来自前端端口的外部总线主机的访问。  
